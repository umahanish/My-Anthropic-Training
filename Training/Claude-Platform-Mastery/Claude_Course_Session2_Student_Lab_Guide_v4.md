...
- `contract-review.skill` (or `.zip`) — Lab 3.3, Step 4
...
3. `"Fix the failing test in this repository."` — use `sample-repo-failing-test.zip` (unzip it first; run `pytest -v` yourself to see the failure before handing it to Claude). Full step-by-step for actually running this one is in Lab 2.3 below.
...
### 🔵 Lab 2.3 — Fix It in the Code Tab *(15 min)*
**Naming note before you start:** "Claude Code" (Anthropic's tool, used below) and "Visual Studio Code" (a separate Microsoft product) are two different things that happen to share a word. You do **not** need Visual Studio Code for this lab — everything happens inside the same Claude Desktop app you've been using all day, in its **Code** tab.

1. Unzip `sample-repo-failing-test.zip` somewhere you'll remember.
2. In Claude Desktop, click the **Code** tab at the top center. *(Prompted to upgrade? The Code tab needs a paid plan — check with your facilitator.)*
3. Choose environment: **Local**.
4. Click **Select folder** and choose the unzipped `sample-repo-failing-test` folder.
   - **Windows:** if prompted that Git is required, install Git for Windows and restart the app before continuing. Most Macs already have Git.
5. Choose **Ask** mode — Claude proposes changes and waits for your approval, rather than editing automatically.
6. Type:
   ```
   Run the test suite and show me why it's failing, then fix the bug.
   ```
7. Review the **visual diff** Claude proposes before approving it — read it the way you'd read a colleague's pull request.
8. After approving, open a terminal and run `pytest -v` yourself to confirm all 3 tests pass now. Don't take a "fixed!" message at face value — verify it, same Discernment habit as everywhere else today.

*(Already a daily Visual Studio Code user? You can alternatively install the official "Claude Code for VS Code" extension and do this same lab there instead — but it's extra setup this lab doesn't require.)*
...
1. Click your name/initials (bottom-left) → **Settings** → **Capabilities** → confirm **"Code execution and file creation"** is toggled **ON**.
2. Click **Customize** → **Skills**. *(Team/Enterprise only: if you don't see this, your organization Owner needs to enable it first in Organization settings > Skills — this does not apply to Free/Pro/Max plans, which go straight here.)*
3. You'll see a list of built-in example Skills (Excel, PowerPoint, Word, PDF creation, etc.) — these are already toggled **on** by default. You don't need to configure anything else to use them.

**Step 2 — Trigger a Built-In Skill**
...
**🔵 Step 4 (Stretch) — Try a Custom Skill**
1. Ask Claude to summarize the 5 sample contracts from your "Contracts" folder, no skill active: `Summarize these vendor contracts.` Notice the output's shape — reasonable, but different every time you'd ask it.
2. Upload the provided `contract-review.skill` file the same way you'd upload any file, via Settings → Capabilities → Customize → Skills → Upload.
3. Ask the exact same question again. Compare: same 5 contracts, same question — what's structurally different about the answer now?
