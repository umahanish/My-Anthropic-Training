...
**Sample files:** A few labs use ready-made sample files instead of requiring your own real material — `IT_Expenses_Sample.xlsx` (Lab 1.2), five files starting with `Contract_...docx` (Labs 2.1, 2.2, 5.2), and `Project_Timeline_Infrastructure_Modernization.docx` (Lab 3.3). All are fictional, built for this course, and safe to use even if you'd rather not upload real company data.
...
2. `"Read every contract in this folder and flag which ones expire in the next 90 days."` *(the same Contracts folder from Lab 2.2 works for this one too, if you want to actually run it — answer: 3 of the 5 contracts fall within 90 days of August 1, 2026)*
...
2. Ask for a real file output:
   ```
   Turn this project timeline into a polished PowerPoint, one slide per
   phase, using a clean and simple design.
   ```
3. Open the result and check formatting quality — real slide structure, not just text dumped onto slides.
4. That polish is a Skill firing automatically. What phrase do you think triggered it?
