...
![MCP architecture: a host runs one client per connected server in a strict 1:1 relationship, and each server exposes tools, resources, and prompts over the same JSON-RPC protocol](images/mcp_architecture.png)
...
![The tool call sequence: client sends tools/list, server returns its tool names, client sends tools/call with arguments, server returns the result — all JSON-RPC 2.0](images/mcp_sequence.png)
...
3. Click the **History** tab — it sits alongside Tools, Resources, and Prompts once you're connected. It logs the raw JSON-RPC request and response for every call you've made through the Inspector so far, usually with a way to copy a request as JSON-RPC directly. Find your `prompts/get` call in the list and open it. You're looking for something shaped like this underneath:
   ```json
   {
     "jsonrpc": "2.0",
     "id": 1,
...
2. Run it: `python contract_server.py`. It should now start an HTTP server (commonly on port 8000) instead of waiting on stdio.
   *(Don't type the URL directly into a browser tab to "check it's working" — a plain browser visit sends a GET request without the `text/event-stream` Accept header Streamable HTTP requires, and the server will correctly reject it with a `-32600` error. That error actually confirms the server is enforcing the protocol properly; it just means you're talking to it the wrong way. Use the Inspector or a real client instead, both below.)*
3. Open the Inspector fresh (`npx @modelcontextprotocol/inspector`, with no server command this time), select **Streamable HTTP** as the transport in the UI, and enter your server's URL — `http://localhost:8000/mcp`, confirmed by actually running this exact server and reading its startup log, not a guess. Connect, and confirm your tool/resource/prompt all still show up — same server code, different transport, same capabilities.
